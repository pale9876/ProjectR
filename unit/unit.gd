...
#
	#var world_2d: World2D = get_viewport().find_world_2d()
	#var space: RID = world_2d.space
#
...
#
		...
		#
		#PhysicsServer2D.area_set_space(_hurtbox.rid, space)
...
#func move(motion: Vector2, delta: float) -> void:
	...
	#
			...
			#shape_param.transform = transform
			#shape_param.motion = motion * delta
			#shape_param.margin = 1.
			#
			#var old_global_position: Vector2 = transform.origin
			#
			#if motion_result.get_collision_safe_fraction() > .1:
				#position += motion * (motion_result.get_collision_safe_fraction() -.01) * delta
			#
			#var remainder: Vector2 = motion_result.get_remainder()
			#var rest_info: Dictionary = world_2d.direct_space_state.get_rest_info(shape_param)
...
#
				...
				#remainder_motion_param.from = transform
				...
				#
					...
					#position += remainder * (remainder_motion_result.get_collision_safe_fraction() -.01)
				#else:
					#position += remainder
			#
			#if mode == FLOAT:
...
#func create_hitbox(duration: float, timer_scale: float) -> void:
	...
	#var space: RID = get_viewport().find_world_2d().space
	#
